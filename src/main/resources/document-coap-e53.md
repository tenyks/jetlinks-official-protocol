# 1. 协议能力说明
* 支持配置式消息结构；
× 支持通信交互策略；
  
# 2. 兼容协议
E53 IAx板通信协议

## 2.1 E53 IA2消息报文字节结构
|名称|值类型|说明|
|---|---|---|---|
|MagicId|UINT16|E53 IA2板：`0xFA11`上线消息, `0xFA37`下行消息|
|消息ID|UINT16|大端在前|
|消息编码|UINT8|参考物模型定义|
|输入/输出参数负载总字节数|UINT16||
|参数1|参考响应消息物模型的定义||
|参数1|参考响应消息物模型的定义||

# 3. 物模型

## 3.1 属性

|名称|编码|值类型|说明|
|---|---|---|---|
|温度|temperature|float|单位：摄氏度|
|相对湿度|humidity|float|单位：RH%|
|亮度|luminance|float|单位：流明|
|低水位标志|lowWaterMark|byte|0=不是，1=是|
|高水位标志|highWaterMark|byte|0=不是，1=是|

## 3.2 服务

### A. 上报数据
开启给水，当达到高水位会自动停止。
* 编码： `ReportData`
* 消息ID标识：`0x10`
* 输入参数

|名称|编码|类型|说明|
|---|---|---|---|
|温度|temperature|float|参数响应属性|
|相对湿度|humidity|float|参数响应属性|
|亮度|luminance|float|参数响应属性|
|低水位标志|lowWaterMark|byte|参数响应属性|
|高水位标志|highWaterMark|byte|参数响应属性|

* 输出参数
  __空__

### B. 开启给水指令
开启给水，当达到高水位会自动停止。
* 编码： `PumpInWaterOn`
* 消息ID标识：`0x11`
* 输入参数

|名称|编码|类型|是否必要|说明|
|---|---|---|---|---|
|工作挡位|degree|UINT8|是|取值范围：(0, 100]，单位：百分比|
|工作时长|duration|UINT32|是|取值范围：(0, 3600000]，最长1小时，单位：毫秒|

* 输出参数
__空__

### C. 关停给水指令
关停给水。
* 编码： `PumpInWaterOff`
* 消息ID标识：`0x12`
* 输入参数：
  __空__
* 输出参数：
  __空__

### D. 开启排水指令
开启排水，可按需设定到达低水位会自动停止。
* 编码： `PumpOutWaterOn`
* 消息ID标识：`0x13`
* 输入参数

|名称|编码|类型|是否必要|说明|
|---|---|---|---|---|
|工作挡位|degree|UINT8|是|取值范围：(0, 100]，单位：百分比|
|工作时长|duration|UINT32|是|取值范围：(0, 3600000]，最长1小时，单位：毫秒|
|低水位时自动停止|autoStopAtLWM|UINT32|否|是否到达低水位时自动停止，`0`表示否，`1`表示是，默认值为：`0`|

* 输出参数
  __空__

### E. 关停排水指令
关停给水。
* 编码： `PumpOutWaterOff`
* 消息ID标识：`0x14`
* 输入参数：
  __空__
* 输出参数：
  __空__

### F. 开启送风指令
开启送风。
* 编码： `FanInAirOn`
* 消息ID标识：`0x15`
* 输入参数

|名称|编码|类型|是否必要|说明|
|---|---|---|---|---|
|工作挡位|degree|UINT8|是|取值范围：(0, 100]，单位：百分比|
|工作时长|duration|UINT32|是|取值范围：(0, 3600000]，最长1小时，单位：毫秒|

* 输出参数
  __空__

### G. 关停送风指令
关停给水。
* 编码： `FanInAirOff`
* 消息ID标识：`0x16`
* 输入参数：
  __空__
* 输出参数：
  __空__

### H. 开启排风指令
开启排风。
* 编码： `FanOutAirOn`
* 消息ID标识：`0x17`
* 输入参数

|名称|编码|类型|是否必要|说明|
|---|---|---|---|---|
|工作挡位|degree|UINT8|是|取值范围：(0, 100]，单位：百分比|
|工作时长|duration|UINT32|是|取值范围：(0, 3600000]，最长1小时，单位：毫秒|

* 输出参数
  __空__

### I. 关停排风指令
关停排风。
* 编码： `FanInAirOff`
* 消息ID标识：`0x18`
* 输入参数：
  __空__
* 输出参数：
  __空__

### J. 开启加热器A指令
开启加热器A。
* 编码： `HeaterAOn`
* 消息ID标识：`0x19`
* 输入参数

|名称|编码|类型|是否必要|说明|
|---|---|---|---|---|
|工作挡位|degree|UINT8|是|取值范围：(0, 100]，单位：百分比|
|工作时长|duration|UINT32|是|取值范围：(0, 3600000]，最长1小时，单位：毫秒|

* 输出参数
  __空__

### K. 关停加热器A指令
关停加热器A。
* 编码： `HeaterAOff`
* 消息ID标识：`0x1A`
* 输入参数：
  __空__
* 输出参数：
  __空__

### L. 开启加热器B指令
开启加热器B。
* 编码： `HeaterBOn`
* 消息ID标识：`0x1B`
* 输入参数

|名称|编码|类型|是否必要|说明|
|---|---|---|---|---|
|工作挡位|degree|UINT8|是|取值范围：(0, 100]，单位：百分比|
|工作时长|duration|UINT32|是|取值范围：(0, 3600000]，最长1小时，单位：毫秒|

* 输出参数
  __空__

### M. 关停加热器B指令
关停加热器B。
* 编码： `HeaterBOff`
* 消息ID标识：`0x1C`
* 输入参数：
  __空__
* 输出参数：
  __空__

### N. 开启补光指令
开启补光。
* 编码： `LightOn`
* 消息ID标识：`0x1D`
* 输入参数

|名称|编码|类型|是否必要|说明|
|---|---|---|---|---|
|工作挡位|degree|UINT8|是|取值范围：(0, 100]，单位：百分比|
|工作时长|duration|UINT32|是|取值范围：(0, 3600000]，最长1小时，单位：毫秒|

* 输出参数
  __空__

### O. 关停补光指令
关停补光。
* 编码： `LightOff`
* 消息ID标识：`0x1E`
* 输入参数：
  __空__
* 输出参数：
  __空__
  
## 事件



# 测试样例
* 上报的心跳事件：
  `fe 00 02 01 ff fd a5 35 00 01 00 02 00 03 00 04 00 05 00 06 3e`
  `000100020003000400050006`
* 游戏结束事件：`fe 00 00 01 ff ff 0c 33 01 00 00 3e`

* BETA环境：`59.36.146.21:5905`